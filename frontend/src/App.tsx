...
function readRecentSessions(): SessionMeta[] {
  try {
    ...
    // Backward-compatibility migration from old string[] format.
...
function App() {
  ...
  const handleNewSession = async () => {
    // Do not clear old session on "New Session".
    // Production UX expects previous chats to remain available in history.
  ...
  const handleOpenSession = async (sessionId: string) => {
    ...
    try {
      ...
      // Keep session visible in recents and preserve a human title.
    ...
    <div className="flex h-screen bg-[#050505] text-foreground overflow-hidden dark">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <div className="px-4 md:px-12 pt-3">
          <div className="max-w-4xl mx-auto text-xs rounded-lg border border-indigo-500/20 bg-indigo-500/5 px-3 py-2 text-indigo-200">
            Frontend runtime: <span className="font-semibold">BACKEND API MODE</span> | Session:{" "}
            <span className="font-mono">{currentSessionId}</span> | Last call:{" "}
